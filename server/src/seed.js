...
/** Published local QA accounts — one pair per product company (login is global by email). */
export const TENANT_DEMO_ACCOUNTS = [
  ...
  },
  {
    ...
    admin: {
      name: 'Admin RAI Documente',
      email: 'admin@rai-spedition.ro',
      password: 'admin123',
      role: 'admin',
    },
    driver: {
      name: 'Șofer RAI',
      email: 'sofer@rai-spedition.ro',
      password: 'sofer123',
      role: 'driver',
      phone: '+40 770 450 496',
    },
  },
];
...
async function upsertUser(client, { companyId, name, email, password, role }) {
  ...
  if (existing.rows[0]) {
    await client.query(
      `UPDATE users
       SET company_id = $1, name = $2, password_hash = $3, role = $4,
           is_active = TRUE, updated_at = NOW()
       WHERE id = $5`,
      [companyId, name, passwordHash, role, existing.rows[0].id]
    );
...
async function ensureDriverProfile(client, { companyId, userId, name, email, phone }) {
  const existing = await client.query(
    `SELECT id FROM drivers WHERE company_id = $1 AND LOWER(email) = LOWER($2)`,
    [companyId, email]
  );
  if (existing.rows[0]) {
    await client.query(
      `UPDATE drivers
       SET user_id = $1, name = $2, phone = COALESCE($3, phone), is_active = TRUE, status = COALESCE(status, 'disponibil')
       WHERE id = $4`,
      [userId, name, phone || null, existing.rows[0].id]
    );
    return existing.rows[0].id;
  }
  const result = await client.query(
    `INSERT INTO drivers (
       company_id, user_id, name, email, phone, hire_date,
       license_number, license_category, license_expiry, status, is_active
     ) VALUES (
       $1, $2, $3, $4, $5, CURRENT_DATE - 100,
       'TMP', 'C', CURRENT_DATE + 365, 'disponibil', TRUE
     ) RETURNING id`,
    [companyId, userId, name, email, phone || null]
  );
  return result.rows[0].id;
}

async function seedFleetForTms(client, companyId, driverUserId) {
  ...
  if (vehicleCount.rows[0].c > 0) {
    await client.query(
      `UPDATE drivers SET user_id = $1
       WHERE company_id = $2 AND LOWER(email) = 'sofer@transitix.ro' AND (user_id IS NULL OR user_id <> $1)`,
      [driverUserId, companyId]
    );
    console.log('TMS fleet already present — ensured driver user link.');
    return;
  }
  ...
  await client.query(
    ...
    [companyId, d1.rows[0].id, v2.rows[0].id]
  );
  ...
  await client.query(
    ...
    [companyId, d1.rows[0].id, v1.rows[0].id]
  );
  ...
  console.log('TMS demo fleet + trips seeded.');
}

async function seed() {
  // Create / refresh product companies before opening a long transaction.
  await ensureProductCompanies(async (sql, params) => pool.query(sql, params));
  ...
  try {
    ...
    const companies = await client.query(
      `SELECT id, name, feature_flags->>'app_key' AS app_key
       FROM companies
       WHERE is_platform = FALSE
       ORDER BY created_at`
    );
    const byKey = new Map(companies.rows.map((r) => [r.app_key, r]));

    console.log('Tenant demo accounts:');
    for (const tenant of TENANT_DEMO_ACCOUNTS) {
      const company = byKey.get(tenant.appKey);
      if (!company) {
        console.warn(`  skip ${tenant.appKey} — company missing (run platform ensure first)`);
        continue;
      }

      await upsertUser(client, { companyId: company.id, ...tenant.admin });
      const driverUserId = await upsertUser(client, { companyId: company.id, ...tenant.driver });
      await ensureDriverProfile(client, {
        companyId: company.id,
        userId: driverUserId,
        name: tenant.driver.name,
        email: tenant.driver.email,
        phone: tenant.driver.phone,
      });

      console.log(`  [${company.name}]`);
      console.log(`    Admin:  ${tenant.admin.email} / ${tenant.admin.password}`);
      console.log(`    Șofer:  ${tenant.driver.email} / ${tenant.driver.password}`);

      if (tenant.appKey === 'transitix_full') {
        await seedFleetForTms(client, company.id, driverUserId);
      }
    }

    await client.query('COMMIT');
    console.log('Seed completed.');
    console.log('  Platform GOD (if bootstrapped): platform@transitix.ro');
  } catch (err) {
